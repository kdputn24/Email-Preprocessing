...
def count_words(email_path='data/enron'):
    ...
    for (dirpath, dirnames, filenames) in os.walk(email_path):
        emails += [os.path.join(dirpath, files) for files in filenames if files[-4:] == '.txt']
    for i in range(len(emails)):
        with open(emails[i], encoding='latin-1') as files:
            content = tokenize_words(files.read())
            for j in content:
                if j in word_freq:
                    word_freq[j] += 1
                else:
...
def find_top_words(word_freq, num_features=200, no_stop_words = False):
    ...
    no_stop_words: bool. False if you want to include the top 32 stop words, True if you don't want to include them
    ...
    # if no stop words
        ...
        counts = []
        sorted_word_freq = sorted(word_freq, key = word_freq.get, reverse= True)
        stop_words = ['the', 'to', 'and', 'of', 'a', 'in', 'for', 'you', 'is', 'enron', 'this', 'on', 'i', 'that', 's', 'subject', 'with', 'be', 'your', 'we', 'as', 'from', 'have', 'it', 'will', 'ect', 'are', 'or', 'at', 'by', 'not', 'our', 'if', 'com']
...
def make_feature_vectors(top_words, num_emails, email_path='data/enron'):
    ...
    for (dirpath, dirnames, filenames) in os.walk(email_path, topdown=True):
    ...
    for name in file_names:
        if "ham" in name:
            y[email_count-1] = 1
        newfile = open(name, encoding='latin-1')
        ...
        for line in newfile:   
            ...
            for word in words:
                ...
                if word in top_words:
                    word_indices = np.argwhere(top_words == word)
                    feats[email_count-1, word_indices] +=1
        email_count+=1
...
def make_train_test_sets(features, y, test_prop=0.2, shuffle=True):
    ...
    # num of each based off given proportion
    num_test = int(len(y)*test_prop)
    ...
    for i in os.listdir(email_path):
        if i != '.DS_Store':
            for name in os.listdir(os.path.join(email_path, i)):
                idx = os.path.join(email_path, i, name)
